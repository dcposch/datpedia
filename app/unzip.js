...
class ZipRandomAccessReader extends yauzl.RandomAccessReader {
  ...
  _readStreamForRange (start, end) {
    ...
    this._readBufsForRange(start, end - 1)
      .then((pages) => {
   ...
   */
  async _readBufsForRange (start, end) {
    ...
    for (let page = pageStart; page <= pageEnd; page++) {
      ...
      if (promise == null) {
        promise = _pagePromiseCache[page] = this._readPage(page)
      }
    }
  ...
  async _readPage (page) {
    ...
    const res = await window.fetch(this._zipPath, { headers })
